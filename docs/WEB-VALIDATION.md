...
- TypeScript 严格类型检查通过。
- 36 个单元、契约和集成测试通过：本地时区日期与进度、本地任务分析、旧工作线与恢复字段迁移、任务日期分组与优先排序、状态/日期修改快照及旧版本恢复、相关资料最小外发、证据范围与敏感信息过滤、带 `[K#]` 引用汇报、分段检索、两阶段语义召回与缓存、引用校验、Responses/Chat Completions/Embeddings 请求契约、真实本机网关代理、密钥端点绑定、DOCX 解压预检、内容指纹、审计导出/验证、项目与工作线失效关联修复、加密备份和 IndexedDB 持久化。
- Vite 生产构建通过。
- `pnpm run health` 通过：Node 版本、依赖、生产构建、运行目录和 4173 端口均满足启动条件。
- PWA manifest、Service Worker 和离线预缓存生成成功。
- Edge 无头浏览器烟雾测试通过：
  - 桌面工作线首页可见，原始要求、澄清、行动、证据与交付节点完整呈现。
  - 设置页的入职边界、模型双协议、检索模式、向量模型、诊断和审计入口可见。
  - 工作区健康检查、审计文件验证入口可见。
  - 交付中心的起止日期在卡片内无横向溢出。
...
- `enableos-web-preview.png`：桌面工作线底稿。
- `enableos-settings.png`：入职边界与模型网关设置。
- `enableos-report-center.png`：日期布局修复后的交付中心。
...
- `enableos-web-mobile.png`：移动端工作线。
