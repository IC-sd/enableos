...
export function GlobalSearch({ open, database, onClose, onNavigate }: { open: boolean; database: AppDatabase; onClose: () => void; onNavigate: NavigateHandler }) {
  ...
  const allResults = useMemo<SearchResult[]>(() => [
    ...
    ...database.reports.filter((item) => !item.deletedAt).map((item) => ({ id: item.id, view: 'reports' as const, type: '交付', title: item.title, detail: item.content, icon: FileChartColumn })),
  ], [database]);
  ...
  return <div className="search-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="global-search" role="dialog" aria-modal="true" aria-label="搜索工作空间"><div className="global-search-input"><Search size={20} /><input ref={inputRef} value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Escape') onClose(); if (event.key === 'Enter' && results[0]) choose(results[0]); }} placeholder="搜索任务、项目、证据、实验或交付……" /><kbd>Esc</kbd></div><div className="global-search-results">{results.length ? results.map((result) => { const Icon = result.icon; return <button key={`${result.view}-${result.id}`} onClick={() => choose(result)}><span className="search-result-icon"><Icon size={17} /></span><div><strong>{result.title}</strong><p>{result.detail.slice(0, 110) || '暂无摘要'}</p></div><em>{result.type}</em></button>; }) : <div className="search-empty">没有找到匹配内容</div>}</div><footer>Enter 打开首项 · 搜索仅在当前浏览器内进行</footer></section></div>;
}
