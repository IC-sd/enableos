...
const navigation = [
  { key: 'dashboard' as const, label: '工作线', short: '线', icon: ListTree },
  ...
  { key: 'reports' as const, label: '交付', short: '交', icon: FileChartColumn },
];
...
export function Sidebar({ database, view, onNavigate, onQuickCapture }: { database: AppDatabase; view: ViewKey; onNavigate: NavigateHandler; onQuickCapture: () => void }) {
  ...
  return (
    <aside className="sidebar rail-navigation">
      <button className="rail-brand" onClick={() => onNavigate('dashboard')} aria-label="EnableOS 工作线">
        <span>E</span><i>/</i>
      </button>

      <button className="rail-capture" onClick={onQuickCapture} aria-label="记录新要求"><Plus size={20} /><span>新建</span></button>
