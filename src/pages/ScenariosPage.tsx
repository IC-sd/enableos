...
import { activity, buildCompanyContext, clampScore, randomUUID } from '../lib/utils';
import { isActive, prependRevision, restoreEntityRevision } from '../lib/entity-history';
import { HistoryPanel } from '../components/HistoryPanel';

const scenarioStatus: Record<ScenarioStatus, string> = { discovered: '已发现', validating: '验证中', prototype: '原型中', adopted: '已落地', archived: '已归档' };
...
export function ScenariosPage({ initialSelectedId }: { initialSelectedId?: string }) {
  ...
  const analyze = async () => {
    ...
    setBusy(true);
    try {
      const response = await desktop.ai.analyzeScenario(description, buildCompanyContext(database, description));
      ...
      const scenario: Scenario = {
        ...
        risk: result.risk, frequency: '待访谈确认', status: 'discovered', hypothesis: `如果使用“${result.aiOpportunity}”，应能改善当前痛点且不降低输出可靠性。`, baseline: '',
      ...
      mutate((current) => ({ ...current, scenarios: [scenario, ...current.scenarios], activities: [activity('scenario', '记录候选AI场景', scenario.title, scenario.id), ...current.activities] }));
    ...
    } catch (error) { notify(error instanceof Error ? error.message : '场景分析失败', 'error'); }
  ...
  const updateStatus = (status: ScenarioStatus) => {
    if (!selected) return;
    patchScenario(selected.id, { status }, `场景进入${scenarioStatus[status]}`);
    notify('场景状态已更新');
  };
  ...
  const recordRun = () => {
    ...
    patchScenario(selected.id, { runs: [run, ...selected.runs], status: selected.status === 'discovered' ? 'validating' : selected.status }, '完成场景评测');
  ...
  const remove = () => {
    if (!selected || !window.confirm(`将场景“${selected.title}”移入回收站吗？`)) return;
    const deletedAt = new Date().toISOString();
    mutate((current) => ({ ...current, scenarios: current.scenarios.map((scenario) => scenario.id === selected.id ? { ...scenario, deletedAt, updatedAt: deletedAt } : scenario), revisions: prependRevision(current, 'scenario', selected, 'delete'), activities: [activity('scenario', 'AI场景移入回收站', selected.title, selected.id), ...current.activities] })); setSelectedId(null); notify('场景已移入回收站', 'info');
  };

  return <div className="page scenarios-page">
    <header className="page-header"><div><p className="eyebrow">AI Experiments</p><h1>AI 场景与评测</h1><p>从真实问题出发，用固定测试集比较每次尝试，再依据证据决定采用、迭代或停止。</p></div><button className="primary-button" onClick={() => setCreateOpen(true)}><Plus size={17} />记录业务问题</button></header>
    <section className="scenario-guidance"><div className="guidance-icon"><Sparkles size={21} /></div><div><strong>实验链路</strong><p>真实痛点 → 当前基线 → 可检验假设 → 固定用例 → 多次运行 → 决策理由。没有测试结果，不把Demo写成落地成果。</p></div></section>
    {sorted.length ? <div className="scenario-grid">{sorted.map((scenario) => {
      ...
      return <button className="scenario-card" key={scenario.id} onClick={() => openScenario(scenario)}><div className="scenario-card-head"><span className={`scenario-status ${scenario.status}`}>{scenarioStatus[scenario.status]}</span><span className={`risk-label ${scenario.risk}`}><ShieldAlert size={13} />{scenario.risk === 'high' ? '高风险' : scenario.risk === 'medium' ? '中风险' : '低风险'}</span></div><h2>{scenario.title}</h2><p>{scenario.pain}</p><div className="scenario-score-row"><div><span>{latest ? '最近评测' : '机会建议'}</span><strong>{latest?.averageScore ?? total}</strong></div><div className="score-bars"><label>价值 <i><b style={{ width: `${scenario.valueScore}%` }} /></i></label><label>可行 <i><b style={{ width: `${scenario.feasibilityScore}%` }} /></i></label><label>数据 <i><b style={{ width: `${scenario.dataReadiness}%` }} /></i></label></div></div><footer><span>{scenario.testCases.length} 用例 · {scenario.runs.length} 次运行 · {decisionNames[scenario.decision]}</span><ArrowRight size={16} /></footer></button>;
    })}</div> : <EmptyState icon={Lightbulb} title="还没有候选场景" description="把业务人员的原话或当前工作流程放进来，先判断是否真的适合使用AI。" action={<button className="primary-button" onClick={() => setCreateOpen(true)}><Plus size={16} />记录第一个业务问题</button>} />}

    <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="记录一个业务问题" description="不需要先想解决方案，尽量保留业务人员的原话。" size="large"><div className="form-grid"><label className="field span-2"><span>业务描述</span><textarea autoFocus rows={8} value={description} onChange={(event) => setDescription(event.target.value)} placeholder="例如：售后每天要从多个群里收集客户反馈，再手工分类并登记到Excel，经常遗漏……" /></label><label className="field"><span>关联项目</span><select value={projectId} onChange={(event) => { setProjectId(event.target.value); setTaskId(''); }}><option value="">暂不关联</option>{database.projects.filter(isActive).map((project) => <option value={project.id} key={project.id}>{project.title}</option>)}</select></label><label className="field"><span>关联工作线</span><select value={taskId} onChange={(event) => { const nextTask = database.tasks.find((task) => task.id === event.target.value); setTaskId(event.target.value); if (nextTask?.projectId) setProjectId(nextTask.projectId); }}><option value="">项目级场景</option>{database.tasks.filter((task) => isActive(task) && (!projectId || task.projectId === projectId)).map((task) => <option value={task.id} key={task.id}>{task.title}</option>)}</select></label><label className="field span-2"><span>相关部门</span><input value={department} onChange={(event) => setDepartment(event.target.value)} placeholder="不知道可以留空" /></label></div><div className="analysis-note"><Gauge size={17} /><span>系统会生成场景卡和验证计划，不会直接把所有问题都判断为需要Agent。</span></div><div className="modal-actions"><button className="secondary-button" onClick={() => setCreateOpen(false)}>取消</button><button className="primary-button" disabled={!description.trim() || busy} onClick={() => void analyze()}>{busy ? <LoaderCircle className="spin" size={16} /> : <Sparkles size={16} />}{busy ? '正在分析' : '生成场景卡'}</button></div></Modal>
    ...
    <Modal open={Boolean(selected)} onClose={() => setSelectedId(null)} title={selected?.title || ''} description={selected ? `${database.projects.find((item) => item.id === selected.projectId)?.title || '未关联项目'} · ${scenarioStatus[selected.status]}` : ''} size="large">{selected ? <div className="scenario-detail">
      <div className="scenario-score-summary"><Score label="业务价值" value={selected.valueScore} /><Score label="实施可行性" value={selected.feasibilityScore} /><Score label="数据准备度" value={selected.dataReadiness} /></div>
      {scenarioDraft ? <details className="experiment-form scenario-core-editor"><summary><Edit3 size={15} /> 编辑完整场景定义</summary><div className="form-grid"><label className="field span-2"><span>场景名称</span><input value={scenarioDraft.title} onChange={(event) => setScenarioDraft({ ...scenarioDraft, title: event.target.value })} /></label><label className="field"><span>关联项目</span><select value={scenarioDraft.projectId || ''} onChange={(event) => setScenarioDraft({ ...scenarioDraft, projectId: event.target.value || null, taskId: null })}><option value="">暂不关联</option>{database.projects.filter(isActive).map((project) => <option value={project.id} key={project.id}>{project.title}</option>)}</select></label><label className="field"><span>关联工作线</span><select value={scenarioDraft.taskId || ''} onChange={(event) => { const task = database.tasks.find((item) => item.id === event.target.value); setScenarioDraft({ ...scenarioDraft, taskId: event.target.value || null, projectId: task?.projectId || scenarioDraft.projectId }); }}><option value="">项目级场景</option>{database.tasks.filter((task) => isActive(task) && (!scenarioDraft.projectId || task.projectId === scenarioDraft.projectId)).map((task) => <option value={task.id} key={task.id}>{task.title}</option>)}</select></label><label className="field"><span>相关部门</span><input value={scenarioDraft.department} onChange={(event) => setScenarioDraft({ ...scenarioDraft, department: event.target.value })} /></label><label className="field"><span>发生频率</span><input value={scenarioDraft.frequency} onChange={(event) => setScenarioDraft({ ...scenarioDraft, frequency: event.target.value })} /></label><label className="field span-2"><span>真实痛点</span><textarea rows={3} value={scenarioDraft.pain} onChange={(event) => setScenarioDraft({ ...scenarioDraft, pain: event.target.value })} /></label><label className="field span-2"><span>当前流程 / 人工基线</span><textarea rows={3} value={scenarioDraft.currentProcess} onChange={(event) => setScenarioDraft({ ...scenarioDraft, currentProcess: event.target.value })} /></label><label className="field span-2"><span>AI 机会边界</span><textarea rows={3} value={scenarioDraft.aiOpportunity} onChange={(event) => setScenarioDraft({ ...scenarioDraft, aiOpportunity: event.target.value })} /></label><label className="field"><span>输入</span><textarea rows={3} value={scenarioDraft.inputs} onChange={(event) => setScenarioDraft({ ...scenarioDraft, inputs: event.target.value })} /></label><label className="field"><span>输出</span><textarea rows={3} value={scenarioDraft.outputs} onChange={(event) => setScenarioDraft({ ...scenarioDraft, outputs: event.target.value })} /></label><label className="field"><span>业务价值 0—100</span><input type="number" min="0" max="100" value={scenarioDraft.valueScore} onChange={(event) => setScenarioDraft({ ...scenarioDraft, valueScore: Number(event.target.value) })} /></label><label className="field"><span>实施可行性 0—100</span><input type="number" min="0" max="100" value={scenarioDraft.feasibilityScore} onChange={(event) => setScenarioDraft({ ...scenarioDraft, feasibilityScore: Number(event.target.value) })} /></label><label className="field"><span>数据准备度 0—100</span><input type="number" min="0" max="100" value={scenarioDraft.dataReadiness} onChange={(event) => setScenarioDraft({ ...scenarioDraft, dataReadiness: Number(event.target.value) })} /></label><label className="field"><span>风险</span><select value={scenarioDraft.risk} onChange={(event) => setScenarioDraft({ ...scenarioDraft, risk: event.target.value as Scenario['risk'] })}><option value="low">低</option><option value="medium">中</option><option value="high">高</option></select></label><label className="field"><span>可检验假设</span><textarea rows={4} value={scenarioDraft.hypothesis} onChange={(event) => setScenarioDraft({ ...scenarioDraft, hypothesis: event.target.value })} /></label><label className="field"><span>量化基线</span><textarea rows={4} value={scenarioDraft.baseline} onChange={(event) => setScenarioDraft({ ...scenarioDraft, baseline: event.target.value })} /></label><label className="field"><span>原型计划（每行一项）</span><textarea rows={5} value={scenarioDraft.prototypePlan.join('\n')} onChange={(event) => setScenarioDraft({ ...scenarioDraft, prototypePlan: event.target.value.split('\n') })} /></label><label className="field"><span>成功指标（每行一项）</span><textarea rows={5} value={scenarioDraft.successMetrics.join('\n')} onChange={(event) => setScenarioDraft({ ...scenarioDraft, successMetrics: event.target.value.split('\n') })} /></label></div><button className="primary-button" onClick={saveScenarioCore}><Save size={15} />保存场景定义</button></details> : null}
      <div className="detail-columns"><section className="detail-block"><label>真实痛点</label><p>{selected.pain}</p></section><section className="detail-block"><label>当前流程</label><p>{selected.currentProcess}</p></section></div>
      <section className="detail-block opportunity"><label>AI机会边界</label><p>{selected.aiOpportunity}</p></section>
      <div className="detail-columns"><section className="detail-block"><label>可检验假设</label><p>{selected.hypothesis || '待补充'}</p></section><section className="detail-block"><label>人工/现有流程基线</label><p>{selected.baseline || '待补充量化基线'}</p></section></div>
