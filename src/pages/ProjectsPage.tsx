...
export function ProjectsPage({ initialSelectedId }: { initialSelectedId?: string }) {
  ...
  const createProject = () => {
    ...
    const project: Project = {
      ...
      progress: 0, dueDate: draft.dueDate, tags: [], deliverables: [], risks: [], nextAction: '确认目标、交付物和第一步行动', createdAt: now, updatedAt: now, deletedAt: '',
    };
  ...
  const exportDossier = async () => {
    ...
    const lines = [
      ...
      '## AI实验', '', ...(relatedScenarios.length ? relatedScenarios.map((item) => `- ${item.title}｜${item.testCases.length}个用例｜${item.runs.length}次运行｜决策：${item.decision}`) : ['- 暂无']), '',
      '## 已保存交付', '', ...(relatedReports.length ? relatedReports.map((item) => `- ${item.title}（${item.rangeStart}—${item.rangeEnd}）`) : ['- 暂无']), '',
    ...
    return <div className="page project-detail-page">
      ...
      <div className="project-detail-grid">
        ...
        <aside className="project-side-stack">
          ...
          <section className="panel"><div className="panel-header"><div><p className="eyebrow">交付与风险</p><h2>项目边界</h2></div></div>
            <div className="boundary-block"><label>预期交付</label>{selected.deliverables.length ? <ul>{selected.deliverables.map((item) => <li key={item}>{item}</li>)}</ul> : <p>可从任务收件箱升级项目，自动带入建议交付物。</p>}</div>
  ...
  return <div className="page">
    ...
    })}</div> : <EmptyState icon={BriefcaseBusiness} title="还没有工作项目" description="任务持续超过一天、需要多项交付或多人协作时，就值得建立项目。" action={<button className="primary-button" onClick={() => setCreateOpen(true)}><Plus size={16} />创建第一个项目</button>} />}
    <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="新建工作项目" description="先写清目标即可，细节可以在执行中逐渐补充。">
      <div className="form-grid"><label className="field span-2"><span>项目名称</span><input autoFocus value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} placeholder="例如：首月AI场景调研" /></label><label className="field span-2"><span>项目目标</span><textarea rows={4} value={draft.objective} onChange={(event) => setDraft({ ...draft, objective: event.target.value })} placeholder="完成后，希望产生什么可验证结果？" /></label><label className="field"><span>业务部门</span><input value={draft.department} onChange={(event) => setDraft({ ...draft, department: event.target.value })} placeholder="可稍后填写" /></label><label className="field"><span>目标日期</span><input type="date" value={draft.dueDate} onChange={(event) => setDraft({ ...draft, dueDate: event.target.value })} /></label></div>
