...
export function TodayPage({ onNavigate }: { onNavigate: NavigateHandler }) {
  ...
  const transition = async (task: Task, status: Task['status']) => {
    if (status === 'done' && !await confirm({ title: '确认完成任务', message: `“${task.title}”将标记为已完成并进入汇报记录，之后仍可重新打开。`, confirmLabel: '完成并留痕' })) return;
    mutate((current) => transitionTask(current, task.id, status));
    notify(status === 'done' ? '已完成并留痕' : status === 'doing' ? '已开始推进' : '已重新打开');
  };
