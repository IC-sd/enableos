import { Bot, CheckCircle2, CircleGauge, ClipboardCheck, DatabaseBackup, Download, FileClock, HardDrive, KeyRound, LoaderCircle, RotateCcw, Save, ShieldCheck, Upload, Wrench } from 'lucide-react';
...
export function SettingsPage() {
  ...
  const updateSettings = (patch: Partial<AppDatabase['settings']>) => mutate((current) => ({
    ...
    activities: patch.policyConfirmedAt && patch.policyConfirmedAt !== current.settings.policyConfirmedAt
      ? [activity('system', '确认工作资料与外部AI边界', `规则：${current.settings.externalAiPolicy}；证据范围：${current.settings.externalEvidenceScope}；核对日期：${patch.policyConfirmedAt}`, null), ...current.activities]
      : current.activities,
  }));
  ...
  const submitBackupDialog = async () => {
    ...
    try {
      if (backupMode === 'export') {
        const result = await desktop.files.exportBackup(database, backupPassword);
        if (!result.canceled) notify(`加密备份已保存到 ${result.path}`);
      } else {
  ...
  return <div className="page settings-page">
    <header className="page-header"><div><p className="eyebrow">Workspace controls</p><h1>设置</h1><p>管理工作上下文、模型网关和浏览器本地数据。</p></div></header>
    <div className="settings-sections">
      <section className="panel settings-section"><div className="settings-section-head"><div className="settings-icon blue"><Bot size={19} /></div><div><h2>你的工作身份</h2><p>这些信息会作为任务分析和方案生成的基础上下文。</p></div></div><div className="form-grid"><label className="field"><span>姓名</span><input value={database.profile.name} onChange={(event) => updateProfile({ name: event.target.value })} placeholder="怎么称呼你" /></label><label className="field"><span>公司</span><input value={database.profile.company} onChange={(event) => updateProfile({ company: event.target.value })} placeholder="入职后填写" /></label><label className="field"><span>岗位</span><input value={database.profile.role} onChange={(event) => updateProfile({ role: event.target.value })} /></label><label className="field"><span>部门</span><input value={database.profile.department} onChange={(event) => updateProfile({ department: event.target.value })} /></label><label className="field"><span>入职日期</span><input type="date" value={database.profile.onboardingDate} onChange={(event) => updateProfile({ onboardingDate: event.target.value })} /></label></div></section>

      <section className="panel settings-section"><div className="settings-section-head"><div className="settings-icon amber"><ClipboardCheck size={19} /></div><div><h2>入职边界核对</h2><p>把未知信息变成第一周可确认的配置；未确认前，系统不会把工作资料发送给外部模型。</p></div><div className={`readiness-badge ${database.settings.policyConfirmedAt ? 'ready' : ''}`}>{database.settings.policyConfirmedAt ? '已核对' : '待核对'}</div></div><div className="form-grid"><label className="field"><span>公司对外部 AI 的规则</span><select value={database.settings.externalAiPolicy} onChange={(event) => updateSettings({ externalAiPolicy: event.target.value as AppDatabase['settings']['externalAiPolicy'] })}><option value="unknown">尚未确认</option><option value="forbidden">禁止发送公司资料</option><option value="approved-with-rules">按公司规则允许</option></select></label><label className="field"><span>允许进入外部模型的证据</span><select value={database.settings.externalEvidenceScope} onChange={(event) => updateSettings({ externalEvidenceScope: event.target.value as AppDatabase['settings']['externalEvidenceScope'] })}><option value="public-only">仅公开资料（默认）</option><option value="public-and-internal">公开和内部资料</option></select></label><label className="field"><span>核对日期</span><input type="date" value={database.settings.policyConfirmedAt} onChange={(event) => updateSettings({ policyConfirmedAt: event.target.value })} /></label><label className="field"><span>批准使用的平台 / 工具</span><input value={database.settings.approvedTools} onChange={(event) => updateSettings({ approvedTools: event.target.value })} placeholder="例如：公司内网模型；不确定就留空" /></label><label className="field span-2"><span>资料处理规则</span><textarea rows={3} value={database.settings.dataHandlingNotes} onChange={(event) => updateSettings({ dataHandlingNotes: event.target.value })} placeholder="哪些资料可使用、是否需要脱敏、结果保存在哪里、谁可以查看" /></label><label className="field"><span>导师对首月成果的期待</span><textarea rows={3} value={database.settings.mentorExpectation} onChange={(event) => updateSettings({ mentorExpectation: event.target.value })} placeholder="记录导师原话，不自行猜测" /></label><label className="field"><span>汇报节奏</span><textarea rows={3} value={database.settings.reportCadence} onChange={(event) => updateSettings({ reportCadence: event.target.value })} placeholder="例如：每周五书面同步" /></label></div></section>
      ...
      <section className="panel settings-section"><div className="settings-section-head"><div className="settings-icon green"><DatabaseBackup size={19} /></div><div><h2>数据、备份与审计</h2><p>任务、资料、实验和交付默认只保存在当前浏览器；审计导出带逐事件 SHA-256 校验链。</p></div><div className={`readiness-badge ${storagePersistent ? 'ready' : ''}`}>{storagePersistent ? '长期保留' : storagePersistent === false ? '可被清理' : '检测中'}</div></div><div className="data-path"><HardDrive size={17} /><div><span>本地数据位置</span><code>{appInfo.dataPath || '正在读取…'}</code><small>{lastSavedAt ? `最近保存 ${new Intl.DateTimeFormat('zh-CN', { hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(new Date(lastSavedAt))}` : '等待首次修改'} · {isReadOnly ? '当前标签页只读' : '当前标签页可编辑'}</small></div></div>{storagePersistent === false ? <div className="secure-note warning"><ShieldCheck size={16} /><span>浏览器仍可能在空间紧张时清理本站数据。<button className="text-button" onClick={() => void requestPersistentStorage()}>申请长期保留</button></span></div> : null}<div className="secure-note"><ShieldCheck size={16} />备份使用 PBKDF2 + AES-GCM 在浏览器内加密；密码通过遮罩输入框处理，不再使用浏览器原始提示框。</div><div className="data-actions"><button className="secondary-button" onClick={exportBackup}><Download size={16} />导出加密备份</button><button className="secondary-button" onClick={() => void importBackup()}><Upload size={16} />导入备份</button><button className="secondary-button" onClick={() => void exportAudit()}><FileClock size={16} />导出审计记录</button><button className="secondary-button" onClick={() => void verifyAudit()}><ShieldCheck size={16} />验证审计文件</button><button className="danger-text-button" onClick={() => void resetDemo()}><RotateCcw size={16} />恢复示例数据</button></div></section>
