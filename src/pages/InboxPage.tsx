  ...
  return <div className="page">
    ...
    <Modal open={Boolean(selected && editTask)} onClose={closeTask} title={selected?.title || ''} description={selected ? `来源：${selected.source} · ${statusLabels[selected.status]}` : ''} size="large">
      {selected && editTask ? <div className="task-editor">
        <section className="detail-block raw-quote"><label>原始要求（保留原话）</label><blockquote>{selected.rawInput}</blockquote></section>
        ...
        <section className="editor-section"><div className="editor-section-head"><div><p className="eyebrow">澄清记录</p><h2>问题与真实答案</h2></div><button className="secondary-button" onClick={() => setEditTask({ ...editTask, clarificationQuestions: [...editTask.clarificationQuestions, ''], clarificationAnswers: [...editTask.clarificationAnswers, ''] })}><Plus size={14} />增加问题</button></div><div className="clarification-editor-list">{editTask.clarificationQuestions.map((question, index) => <article key={index}><span>{String(index + 1).padStart(2, '0')}</span><div><input aria-label={`澄清问题 ${index + 1}`} value={question} onChange={(event) => updateQuestion(index, 'question', event.target.value)} /><textarea aria-label={`澄清答案 ${index + 1}`} rows={2} value={editTask.clarificationAnswers[index] || ''} onChange={(event) => updateQuestion(index, 'answer', event.target.value)} placeholder="记录导师或业务人员的原话；没有答案可以留空" /></div><button className="icon-button" aria-label={`删除澄清问题 ${index + 1}`} onClick={() => removeQuestion(index)}><X size={14} /></button></article>)}</div></section>
        ...
        <div className="detail-columns"><label className="field"><span>预期交付物（每行一项）</span><textarea rows={5} value={editTask.deliverables.join('\n')} onChange={(event) => setEditTask({ ...editTask, deliverables: event.target.value.split('\n') })} /></label><label className="field"><span>验收标准（每行一项）</span><textarea rows={5} value={editTask.acceptanceCriteria.join('\n')} onChange={(event) => setEditTask({ ...editTask, acceptanceCriteria: event.target.value.split('\n') })} placeholder="例如：导师确认可用；10条测试全部通过" /></label></div>
