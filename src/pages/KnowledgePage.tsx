...
import { activity, buildWorkspaceContext, formatFullDate, randomUUID, truncate } from '../lib/utils';
  ...
  const askKnowledge = async () => {
    ...
    try {
      ...
      if (database.settings.aiMode === 'api' && !externalAllowed) {
        ...
        notify('外部 AI 使用边界尚未确认，本次未上传资料。请先在设置中明确数据边界。', 'info');
      } else {
        const response = await desktop.ai.ask({ instruction: question, context: `${buildWorkspaceContext(database, question)}\n\n检索证据：\n${evidenceContext(matches)}` });
    ...
    <Modal open={Boolean(selected)} onClose={() => { setSelectedId(null); setEditDraft(null); }} title={selected?.title || ''} description={selected ? `${selected.sourceName || '手动记录'} · 更新于 ${formatFullDate(selected.updatedAt)}` : ''} size="large">
      {selected && editDraft ? <div className="knowledge-detail">
        <div className="form-grid">
          ...
          <label className="field"><span>权限</span><select value={editDraft.confidentiality} onChange={(event) => setEditDraft({ ...editDraft, confidentiality: event.target.value as Confidentiality })}><option value="public">公开</option><option value="internal">非公开</option><option value="sensitive">敏感</option></select></label>
