...
export function ReportsPage({ initialSelectedId, onNavigate }: { initialSelectedId?: string; onNavigate: NavigateHandler }) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (!title.trim()) {
        ...
        setTitle(task ? `${task.title} · 任务汇报` : project ? `${project.title} · 阶段汇报` : `工作周报 ${rangeStart}—${rangeEnd}`);
      }
  ...
  const save = () => {
    ...
    const report: SavedReport = { id: randomUUID(), projectId: effectiveProjectId || null, taskId: taskId || null, kind: effectiveProjectId ? 'project' : 'weekly', title: title.trim() || (task ? `${task.title} · 任务汇报` : project ? `${project.title} · 阶段汇报` : `工作周报 ${rangeStart}—${rangeEnd}`), rangeStart, rangeEnd, content, createdAt, updatedAt: createdAt, deletedAt: '' };
  ...
  const updateSaved = () => {
    ...
    mutate((current) => ({ ...current, reports: current.reports.map((item) => item.id === previous.id ? { ...item, title: title.trim(), content, rangeStart, rangeEnd, projectId: projectId || null, taskId: taskId || null, updatedAt } : item), revisions: prependRevision(current, 'report', previous, 'update'), activities: [activity('report', '更新汇报版本', title.trim(), previous.id), ...current.activities] }));
    notify('当前版本已更新，并保留修改前快照');
  };

  const removeReport = async () => {
    const previous = database.reports.find((item) => item.id === selectedReportId && isActive(item));
    if (!previous || !await confirm({ title: '移入回收站', message: `汇报“${previous.title}”会移入回收站，之后可以恢复。`, confirmLabel: '移入回收站', tone: 'danger' })) return;
    const deletedAt = new Date().toISOString();
    mutate((current) => ({ ...current, reports: current.reports.map((item) => item.id === previous.id ? { ...item, deletedAt, updatedAt: deletedAt } : item), revisions: prependRevision(current, 'report', previous, 'delete'), activities: [activity('report', '汇报移入回收站', previous.title, previous.id), ...current.activities] }));
    setSelectedReportId(''); setContent(''); setTitle(''); notify('汇报已移入回收站', 'info');
  };
